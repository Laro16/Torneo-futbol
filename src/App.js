...
/*
  Proyecto: Liga Local (Versión Final Corregida)
  - Soluciona el error de parseo que convertía texto a números incorrectamente.
  - Ahora los títulos, fechas, nombres y rondas se mostrarán correctamente.
*/
...
// --- Helpers ---
const safeNumber = (v) => {
  if (v === null || v === undefined || v === '') return null; // Devuelve null para números vacíos
...
// CORRECCIÓN: Lista de encabezados que deben ser tratados como números.
...
const parseCSV = (csvText) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((h, idx) => {
      ...
      // CORRECCIÓN: Solo se convierte a número si el encabezado está en la lista.
      if (NUMERIC_HEADERS.includes(h)) {
        entry[h] = safeNumber(val);
      } else {
        entry[h] = val; // El resto se queda como texto (títulos, nombres, fechas, etc.)
      }
    });
    data.push(entry);
  }
...
const CACHE_PREFIX = 'liga_local_cache_v2_'; // Se cambió la versión del caché para invalidar datos viejos
